...
// Ensure these are installed: npm install slick-carousel react-slick
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";

const Project = (props) => {
  return (
    /* Adjusted padding from px-9 to px-4 on mobile for better spacing */
    <div className="px-4 py-6 sm:px-9 sm:py-9 transition-colors duration-300 transform border rounded-xl hover:border-transparent group dark:border-gray-700 dark:hover:border-transparent feature-card mx-auto max-w-2xl mb-10">
      <div className="flex flex-col sm:flex-row items-center sm:items-start">
        {/* Adjusted image sizing to be more fluid on mobile */}
        <img 
          className="flex-shrink-0 object-cover w-32 h-32 sm:w-40 sm:h-40 rounded-full ring-4 ring-gray-300" 
          src={props.image} 
          alt={props.title} 
        /> 
        <div className="mt-4 sm:mx-6 sm:mt-0 text-center sm:text-left">
          <h1 className="text-2xl font-semibold text-gray-700 capitalize group-hover:text-white text-gradient">
            {props.title}
          </h1>
          <p className="font-poppins font-normal text-dimWhite mt-3">Tech Stack</p>
          <div className="flex justify-center sm:justify-start mt-2">
            {props.stack.map((tech, index) => (
              <div key={index} className="text-dimWhite mr-4 text-[20px] hover:text-teal-200 relative group/icon">
                {React.createElement(tech.icon)}
                <span className="absolute bottom-full mb-2 hidden group-hover/icon:block bg-black text-xs p-1 rounded">
                  {tech.name}
                </span>
              </div>
            ))}
      ...
      <p className="mt-6 text-gray-500 dark:text-gray-300 group-hover:text-gray-300 text-sm sm:text-base leading-relaxed">
        {props.content}
      </p>
      <div className="flex mt-6 justify-center sm:justify-start space-x-4">
        {props.github && (
          <a href={props.github} target="_blank" rel="noopener noreferrer">
            <AiFillGithub size="2rem" className="text-white hover:text-cyan-400 transition-colors" />
          </a>
        )}
        {props.link && (
          <a href={props.link} target="_blank" rel="noopener noreferrer">
            <BsLink45Deg size="2rem" className="text-white hover:text-cyan-400 transition-colors" />
          </a>
        )}
      </div>
    </div>
...
const Projects = () => {
  ...
  // RESPONSIVE SETTINGS: This is the fix for the overlapping cards
  const settings = {
    dots: true,
    infinite: true,
    speed: 500,
    ...
    responsive: [
      ...
      },
      {
        ...
        settings: {
          slidesToShow: 1,
          arrows: false, // Arrows often overflow on narrow mobile screens
          dots: true,
        }
      }
    ]
  ...
  const projects = [
    ...
    },
    {
      ...
      },
      {
        ...
        content: "SkyByte™ brings premium travel to everyone, one bite-sized component at a time. Got us to the finals for the Cathay 2024 Hackathon.",
        github: "https://github.com/tanyabudhrani/SkyByte",
        link: "",
      },
      {
      ...
      {
        title: "Layered Fraud Checker",
        image: "/assets/fraud.jpeg",
        stack: [
          { name: "Python", icon: FaPython },
        ],
        content: "A multi-layer, risk-adaptive fraud detection system built on the Kaggle Credit Card Fraud Dataset.",
        github: "https://github.com/tanyabudhrani/Layered-Fraud-Credit-Checker/tree/main",
        link: "",
      },
      {
        title: "Dont Overfit Challenge",
        image: "/assets/k.png",
        stack: [
          { name: "Python", icon: FaPython },
        ],
        content: "A methodologically sound reimplementation of the classic “Don’t Overfit!” Kaggle competition.",
        github: "https://github.com/tanyabudhrani/Dont-Overfit-Kaggle-Challenge",
        link: "",
      }
  ];
  ...
  return (
    <section id="projects" className="py-20 bg-dark-800 text-white overflow-hidden">
      <div className="container mx-auto px-4">
        {/* Adjusted title size for better mobile fit */}
        <h1 ref={titleRef} className="text-4xl sm:text-6xl font-bold mb-12">
          {showTypewriter && (
            <Typewriter
              words={['Projects']}
              typeSpeed={70}
        ...
        <Slider {...settings}>
          {projects.map((project, index) => (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.5 }}
              className="outline-none" // Removes the blue focus box from slick
            >
              <Project {...project} />
            </motion.div>
          ))}
